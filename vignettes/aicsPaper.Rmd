...
3. Create an RDS file containing information about the samples (AK WDTM?)
...
1KG GDS file. The 1KG GDS file available for download (AK FROM?) is based on the hg38 genome build.
...
The 3 files should be stored in the same directory. For the current (AK IN THE EXAMPLE BELOW?)
demonstration, this directory is referred to as **PATH_REF**.
...
The RDS file must contain a *data.frame* with these 5 columns: (AK NONE OF WHICH SEEMS NECESSARY)
...
- **Source**: The source of the profile sequence data (example: dbGAP_XYZ). (AK REPOSITORY?)
...
### Sub-Step 4. Create a profile GDS file (1 GDS file per profile) (AK SAMPLE -> PROFILE EVERYWHERE?)
...
##############################################################
## The list of samples to analyze.
## The samples must be listed in the Sample PED RDS file. 
...
## to a row the information present in the 
## data.frame defined in the preceding step.
#################################################################
studyDF <- data.frame(study.id = "MYDATA",
                        study.desc = "Description",
                        study.platform = "PLATFORM",
...
for(i in seq_len(length(listSamples))) {
    ...
    ## study.id: Study identifier as defined in the preceding step. (AK EXPLICIT STEP NUMBER)
    ##           The study identifier must be the same that the one present in
    ##           Sample GDS file.
    ## PATHSAMPLEGDS: All Sample GDS files must be in the same directory. (AK IS THIS EXPLAINED IN AN APPROPRIATE PLACE EARLIER?)
...
## A data.frame with the description of the study for the synthetic
## data is required.
## The column names must be as shown below
#################################################################### 
studyDF.syn <- data.frame(study.id = "MYDATA.Synthetic",
                          study.desc = "MYDATA synthetic data",
                          study.platform = "PLATFORM",
...
## as listed in the PED RDS file. The samples must have an 
...
####################################################################
## A data.frame with the description of the data used for synthesis
## is required.
## The column names must be as shown 
#################################################################### 
studyDF.syn <- data.frame(study.id = "MYDATA.Synthetic",
...
####################################################################
## The list of samples to be analyze as given by the PED RDS file.
