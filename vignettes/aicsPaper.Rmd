...
SNVs present in the pruned SNV dataset. The function requires those 2 or 3 files:

1. The 1KG GDS file
2. The GDS Sample file
3. In the case of RNA-seq The 1KG annotation GDS TODOPB
...
## Estimate the allelic fraction of the pruned SNVs for the current sample for DNA sample
## In the case of RNA-seq it need the GDS gds.Annot and
## block.id which referes to block.id in gds.Annot fro the 
## gene annotion of the SNV TODOPB
snp.pos <- estimateAllelicFraction(gds=gds, gdsSample=gdsSample,
                                      sampleCurrent=sampleName, 
                                      study.id=studyDF$study.id,
                                      chrInfo=chrInfo)

## Estimate the allelic fraction of the pruned SNVs for the current sample for RNA-seq sample TODOPB
# snp.pos <- estimateAllelicFraction(gds=gds, gdsSample=gdsSample,
#                                    sampleCurrent=sampleName, 
#                                    study.id=studyDF$study.id,
#                                    chrInfo=chrInfo, 
...
#                                    block.id="GeneS.Ensembl.Hsapiens.v86")
...
library(RAIDS)    
    ...
    # synthetic data
    tudyDF.syn <- data.frame(study.id = "Id of the synthetic dataset",
    ...
    for(i in seq_len(length(listSamples))){
        ...
        for(j in seq_len(nrow(sampleRM))){
            KNN.synt <- computePoolSyntheticAncestryGr(gds, gdsSample,
                                                             sampleRM[j,],
                                                             studyDF.syn$study.id,
                                                             np=4,
                                                             spRef=spRef,
                                                             eigen.cnt=15L)
        ...
        saveRDS(KNN.sample.syn, file.path(PATH_OUT_SAMPLE, paste0("KNN.synt.", listSamples[i], ".", i, ".rds")))
...
The PCA of the sample and KNN sample and Call the ancestry with the optimal parameters K and D

Note The formal selection of parameter K and D  is done here too but all the synthetic data are prepare in 
the step 3. 
...
library(RAIDS)    
    ...
    for(i in seq_len(length(listSamples))){
        ...
        saveRDS(resCall, file.path(PATHOUT, paste0( listSamples[i], ".infoCall",".rds")))
