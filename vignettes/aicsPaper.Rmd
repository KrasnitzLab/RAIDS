...
1. Format the information from 1000 Genomes (1KG)
2. Format the information from an external study to get if ready for ancestry inference
...
Those steps explain how to generate the 1KG GDS file needed to run the 
ancestry inference tool. Beware that it is unnecessary to re-run those 
steps as the 1KG GDS file is publicly available. 

TODO where?

The mains steps of this processus are:
...
4. Create a predigree file in RDS format
...
8. Identify genetically related patients in 1KG
9. Add information about unrelated patients to the 1KG GDS file
10. Create a second GDS containing the 1KG phase information

Beware that hose steps are time as well as space demanding.
...
## The variable chrCur is one specific chromosome (example, chrCur="chr1")
## The variable PATHGENO is a specified directory
## Create one directory for the specific chromosome
mkdir ${PATHGENO}/${chrCur}

## The variable FILECUR is the associated VCF file for the specific chromosome
## FILECUR is the vcf file from 1000 genome
## Example, FILECUR=ALL.chr1.shapeit2_integrated_v1a.GRCh38.20181129.phased.vcf.gz

## There is 2548 samples in the VCF file
## The information associated to each sample is extracted and a VCF file 
## specific to this sample for the current chromosome is generated
for i in `seq 1 2548`
do
    j=$(( $i + 9 ))
...
information (__filePart__ parameter) as well as a RDS file with the kinship 
coefficient between the patients (__fileIBD__ parameter).
...
## Format information from an external study to get if ready for ancestry inference
...
First, you need a mapped BAM file for each sample. The reference genome used 
for the mapping must correspond to the one used to generate the 1KG GDS file. 
In this case, the 1KG GDS file is based on genome hg38.
...
The __snvListVCF()__ function is used to generate a VCF file that will contain
