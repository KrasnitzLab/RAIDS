...
demo_PED_File <- file.path(dataDir, "Demo_PED.RDS")
...
- The **1KG GDS file**
- The **Sample SNP pileup file** (one per profile present in the study)
- The **Sample PED RDS file** (one file with information for all profiles in the study)
...
file.GDS <- file.path(path1KG, fileName.GDS.1KG)
...
for(i in seq_len(length(listSamples))) {
    ...
    ##           sub-step 4. 
    pruningSample(gdsReference=gds_1KG,
    ...
    file.GDSSample <- file.path(pathProfileGDS, paste0(listSamples[i], ".gds"))
    ...
    add1KG2SampleGDS(gdsReference=gds_1KG, fileProfileGDS=file.GDSSample,
                        currentProfile=listSamples[i],
                        studyID=studyDF$study.id)
...
file.GDS <- file.path(path1KG, fileName.GDS.1KG)
file.Annot <- file.path(path1KG, fileName.GDS.Annot)
...
file.GDSSample <- file.path(pathProfileGDS, paste0(sampleName, ".gds"))
...
estimateAllelicFraction(gdsReference=gds, gdsSample=gdsSample,
                                        currentProfile=sampleName,
                                        studyID=studyDF$study.id,
                                        chrInfo=chrInfo)
...
####################################################################
prepSynthetic(fileProfileGDS=file.GDSSample,
...
####################################################################
resG <- syntheticGeno(gdsReference=gds,
                        gdsRefAnnot=gds.Annot,
                        fileProfileGDS=file.GDSSample,
                        data.id.profile=sampleName,
                        listSampleRef=listSampleRef,
                        prefId="1")
...
for(i in seq_len(length(listSamples))) {
    ...
    file.GDSSample <- file.path(pathProfileGDS,
    ...
    for(j in seq_len(nrow(sampleRM))) {
        ...
        KNN.synt <- computePoolSyntheticAncestryGr(gdsSample=gdsSample,
                                            sampleRM= sampleRM[j,],
                                            studyIDSyn=studyDF.syn$study.id,
                                            np=4L,
                                            spRef=spRef,
                                            eigen.cnt=15L)
