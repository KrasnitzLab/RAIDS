...
Two parameters can be optimized in the ancestry inference process:

- K: the number of neighbors used to call the ancestry
- D: the number of dimensions used to create the PCA

The accuracy is evaluated through synthetic dataset created from merging a 
cancer profile with a 1KG sample of known ancestry. Using the synthetic 
profiles, a range of K and D values are tested. Through that process, the K 
and D values with satisfying accuracy can be selected.
...
# The data is saved in the GDS sample file
...
Note: The formal selection of K and D parameters is done at this step but 
