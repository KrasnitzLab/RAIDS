...
test_that(paste0("prepPedSynthetic1KG() must return error when gds is a ",
    ...
    expect_error(prepPedSynthetic1KG(gds=gdsF,  gdsSample="test.gds",
                        study.id="TCGA", popName="superPop"), error_message)
})
...
test_that(paste0("prepPedSynthetic1KG() must return error when study.id is a ",
    ...
    expect_error(prepPedSynthetic1KG(gds=gdsF,  gdsSample=gdsF,
                study.id=23, popName="superPop"), error_message)
})
...
test_that(paste0("prepPedSynthetic1KG() must return expected results"), {
    ...
    dataF <- data.frame(data.id=c("A101TCGA", "HG00101.Synthetic.01", "HG00101.Synthetic.02",
                            "HG00102.Synthetic.02", "HG00109.Synthetic.02"),
                case.id=c("A101TCGA", "HG00101", "HG00101", "HG00102", "HG00109"),
                sample.type=c("TCGA", "Synthetic", "Synthetic", "Synthetic", "Synthetic"),
                diagnosis=rep("C", 5),
                source=c("TCGA", "Synthetic", "Synthetic", "Synthetic", "Synthetic"),
                study.id=c("TCGA", "TCGA.Synthetic", "TCGA.Synthetic", "TCGA.Synthetic", "TCGA.Synthetic"))
    add.gdsn(node=gdsSample, name="study.annot", val=dataF)
    sync.gds(gdsSample)

    result1 <- prepPedSynthetic1KG(gds=gds1KG,  gdsSample=gdsSample,
