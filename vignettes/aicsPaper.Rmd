...
cancer-derived sequencing. The covered cancer-derived sequencing are:
...
Those main steps are described in details in the following sections.
...
This step generates 3 important reference files:
...
3. Create a Sample RDS file containing information about the samples
...
Beware that a mapped BAM file is needed for each sample (step 2). The reference 
genome used for the mapping must correspond to the one used to generate the 
...
demonstration, the directory will be referred as **PATH_REF**.
...
the counts of the reference and alternative nucleotides from an BAM input.
...
alternative nucleotide counts at each SNP location present in the 
...
Beware that the name assigned to the **Sample SNP pileup file** will correspond 
to the Sample identifier (Name.ID) in the following analysis. For example, a 
...
## PATH_REF/snvSel0.01.vcf.gz : The SNP Retained VCF file containing the 
...
### Sub-Step 3. Create a Sample PED RDS file containing the information about the samples

A RDS file describing all the samples that are part of the analysis 
is required.

The RDS file must contain a *data.frame* with those 5 columns:

- **Name.ID**: The unique sample identifier. The Sample VCF file associated to 
...
- **Source**: The source of the sample (example: dbGAP_XYZ).

Important: The row names of the *data.frame* must be the **Name.ID**.

This file is referred as the **Sample PED RDS file** (PED for pedigree). 
...
### Sub-Step 4. Create a Sample GDS file (1 GDS file per sample)
...
```{r appendStudy2GDS1KG, echo=TRUE, eval=TRUE, collapse=TRUE}
#################################################################
## Load required package
#################################################################
library(RAIDS)
...
## Beware that the 1KG GDS file used for this example is a  
...
#################################################################
## The list of samples to analyzed is passed to the function.
## The samples must be present in the Sample PED RDS file
## and must have an associated Sample SNP pileup file.
## Not all samples present in the Sample PED file need to 
## be selected.
#################################################################
...
retained SNVs is added to the **Sample GDS file**.
...
```{r pruningSample, echo=TRUE, eval=TRUE}
##############################################################
## Load required package
##############################################################
library(RAIDS)
...
##############################################################
## All Sample GDS files need to be located in the same 
## directory.
##############################################################
PATHSAMPLEGDS <- file.path(data.dir, "example", "out")

##############################################################
## The list of samples to analyse.
## The samples must be present in the Sample PED RDS file. 
## The samples must also have an associated Sample GDS file 
## (see previous step).
##############################################################
filePED <- file.path(data.dir, "example", "pedEx.rds")
ped <- readRDS(filePED)
...
## to the information present in the previously defined study
...
for(i in seq_len(length(listSamples))) {
    ...
    ##           Sample GDS file.
    ## PATHSAMPLEGDS: All Sample GDS files must be in the same directory.
    pruningSample(gds=gds_1KG,
                    sampleCurrent = listSamples[i],
                    study.id = studyDF$study.id, 
                    PATHSAMPLEGDS = PATHSAMPLEGDS)
    
    ## Sample GDS file for the current sample
...
There are other ways to extract the information. 
...
```{r estimateAllelicFraction, echo=TRUE, eval=TRUE}
###################################################################
## Load required package
###################################################################
library(RAIDS)
...
This step must be run for each sample present in the study.
...
are association to the creation of the synthetic dataset.

Those 3 files are required:
...
```{r generateSynthetic, echo=TRUE, eval=TRUE}
####################################################################
## Load required package
####################################################################
library(RAIDS)    
...
####################################################################
## The prepSynthetic() function prepares the annotations for
## the synthetic data
...
### Sub-Step 2. Compute the PCA-KNN ancestry call for each synthetic profile



```{r PCA.KNN.Synthetic, echo=TRUE, eval=TRUE}
####################################################################
## Load required package
####################################################################
library(RAIDS)    
...
####################################################################
## The path to the directory containing the Sample GDS files
####################################################################
PATHSAMPLEGDS <- file.path(data.dir, "example", "out")
...
####################################################################
## The list of cancer samples used to generate the synthetic data 
## as obtained by the PED RDS file. The samples must have an 
## associated Sample GDS file.
####################################################################
filePED <- file.path(data.dir, "example", "pedEx.rds")
ped <- readRDS(filePED)
listSamples <- ped[, "Name.ID"]
...
```{r PCA.KNN.Sample, echo=TRUE, eval=TRUE}
####################################################################
## Load required package
####################################################################
library(RAIDS)    
...
# directory which content directory name Name.ID in which the PCA is save
# 
PATH_OUT <- file.path(PATHSAMPLEGDS)
...
####################################################################
## The list of samples to analyse as obtained by the PED RDS file.
## The samples must have an associated Sample GDS files.
## Not needed if the list of samples is already available.
####################################################################
filePED <- file.path(data.dir, "example", "pedEx.rds")
ped <- readRDS(filePED)
listSamples <- ped[, "Name.ID"]
