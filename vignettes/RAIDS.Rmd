...
The *inferAncestry()* function requires a specific input format for individual 
genotyping profiles. The format is set by the *genoSource* parameter. 
...
Note that the start position in each chromosome is 0. 
...
Finally, all temporary files created in this example should be deleted.
...
Global ancestry is inferred inference using principal-component decomposition
