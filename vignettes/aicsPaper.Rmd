...
### Sub-Step 2. Generate a Sample VCF file (1 file per sample)
...
Given a VCF file containing SNP locations, 
...
appendStudy2GDS1KG(PATHGENO = PATHGENO,
                      ...
                      batch = 1,  ## Numerical value required
                      studyDF = studyDF,
...
The initial list of 1KG SNPs is pruned, using linkage disequilibrium analysis, 
...
The function __pruningSample()__ requires the 1KG GDS file as input. It also 
requires the path to the GDS Sample files.

Beware that this step is requiring large disk space.

To speed up the process, the function can be run on multiple CPUs.
...
## the the information present in the previously defined 
...
for(i in seq_len(length(listSamples))) {
  ...
  ## np: Number of CPU (Default: 1)
  ## keepGDSpruned: When TRUE, the information about the pruned SNVs is 
  ##                  added to the GDS Sample file
  ## PATHSAMPLEGDS: All GDS Sample files must be in the same directory
  ## keepFile: When TRUE, 2 RDS files with the pruned SNVs information 
  ##                  are generated
  ...
  ## in the GDS Sample file
  add1KG2SampleGDS(gds = gds_1KG, gdsSampleFile = file.GDSSample, 
                      sampleCurrent = listSamples[1], 
                      study.id = studyDF$study.id))
  
  ## Add annotation to the GDS Sample file
  addStudy1Kg(gds_1KG, file.GDSSample)
}
    
## Close the 1KG GDS file (it is important to close the GDS file)
closefn.gds(gds)
```
...
SNVs present in the pruned SNV dataset. The function requires those 2 files:

1. The 1KG GDS file
2. The GDS Sample file

The 3 other inputs are:
...
The function processes one sample at the time. 
...
## Name of the GDS Sample file (the name corresponds to the sample identifier)
file.GDSSample <- file.path(PATHSAMPLEGDS, paste0(sampleName, ".gds"))

## Open the 1KG GDS file  
gdsSample <- openfn.gds(file.GDSSample, readonly = FALSE)

## Estimate the allelic fraction of the pruned SNVs for the current sample
...
### Sub-Step 7. Generate the synthetic dataset
...
# The data is added to the GDS sample file
...
### Sub-Step 8. Compute the PCA-KNN ancestry call for each synthetic data
...
Before running the ancestry inference, the selection of the best parameters 
must be done.

```{r graphStep4, echo = FALSE, fig.align="center", fig.cap="Step 3 - Find the optimized parameters for the ancestry inference", out.width = '120%', results='asis'}
knitr::include_graphics("MainSteps_Step3_v01.png")
```


### Sub-Step 1. Select paramters for the PCA-KNN ancestry call
